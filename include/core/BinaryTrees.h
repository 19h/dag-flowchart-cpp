...
public:
    ...
    void set(size_t index, const NodeType &value) {
        ...
        while (pos > 1) {
            auto parent = pos >> 1;
            this->This().updateFromChildren(this->nodes[parent], this->nodes[pos],
                                            this->nodes[pos ^ 1]);
            pos = parent;
        }
    }
...
public:
    ...
    /**
     * @brief Find right most position with value less than given in range [0; position].
 ...
 * the operation stored in \a promise for nodePosition to the direct children nodes.
 *
